...
namespace NSwag.CodeGeneration.Models
{
       ...
       where TResponseModel : ResponseModelBase
   {
      ...
      /// <summary>
      /// Extract the AuthenticationAttribute
      /// </summary>
      /// <returns>Authentication attribute or empty string</returns>
      public string AuthenticationAttribute
      {
         get
         {
            if (!string.IsNullOrEmpty(Summary) && Summary.Contains("[AllowAnonymous]"))
            {
      ...
      /// <summary>
      /// Get the authorize attribute like [Authorize(Policy="AdminRole"] from arbitrary string
      /// </summary>
      /// <returns>Authorization attribute or emtpy string</returns>
      public string AuthorizationAttribute
      {
         get
         {
            if (string.IsNullOrEmpty(Summary)) return "";
            var rgx = new Regex(@".*(?<polattr>\[\s?Authorize\s?\(\s?Policy.*\]).*");
            var match = rgx.Match(Summary);
            return match.Success ? match.Groups["polattr"].Value : "";
         }
      }
